...
dependencies {
    compileOnly(project(COMMON_PROJECT)) {
        capabilities {
            requireCapability("$group:$modid")
        }
    }
    COMMON_JAVA(project(path = COMMON_PROJECT, configuration = COMMON_JAVA))
    COMMON_RESOURCES(project(path = COMMON_PROJECT, configuration = COMMON_RESOURCES))
}
