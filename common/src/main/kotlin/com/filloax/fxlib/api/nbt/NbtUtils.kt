...
fun <T> CompoundTag.loadField(name: String, codec: Codec<T>): T? {
    return this.get(name)?.let{ tag ->
        try {
            codec.decode(NbtOps.INSTANCE, tag).getOrThrow {
                throw Exception("Error in decoding $name: $it")
            }.first
        } catch (e: Exception) {
...
fun <T> CompoundTag.saveField(name: String, codec: Codec<T>, getter: () -> T?) {
    ...
    if (value != null) {
        try {
            this.put(name, codec.encodeStart(NbtOps.INSTANCE, getter()).getOrThrow {
                throw Exception("Error in encoding $name: $it")
            })
        } catch (e: Exception) {
